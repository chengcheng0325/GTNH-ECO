...
public class ItemEcalThreadCore extends Item {
    ...
    /**
     * t60/t114f→t131 (upgrade tree, docs §2): the thread-chain / hyper node required to insert
     * this core — normal 1 → T1, 4 → T2, ≥16 → T3; hyper by hyperThreads: 4 → H2, 8+ → H3
     * (t114s: hyper_2 supplies 0+4 → H2, hyper_4 4+8 → H3, hyper_8 8+16 → H3).
     * <p>
     * ⚠ 284 divergence (plan §3.7/§6): this version KEEPS the 32/64-thread cores as registered
     * items (a live save may already have one inserted in a drive), so both the 32- and the
     * 64-thread stack are truncated onto T3 — the same node as the 16-thread core. Withdrawing
     * the item would invalidate existing stacks in a live world (授权原则 ②).
     */
