...
/**
 * t65→t128b: the calculator host's upgrade tree (docs/ECO_UPGRADE_TREE_DESIGN.md §2 revision,
 * user decision: single main chain + branches). t128b merges every three consecutive cell nodes
 ...
 * t128b (TEST ONLY, carried over from t113c): every non-free node costs ONE IRON INGOT so the
 * whole tree can be walked quickly in a test world.
 */
public final class CalculatorUpgradeTree extends UpgradeTree {

    // Cell main chain (t128b: 5 nodes — 3 merged groups + 16384M + singularity tails).
    public static final String N1 = "N1";
    public static final String N2 = "N2";
    public static final String N3 = "N3";
    public static final String N4 = "N4";
    public static final String N5 = "N5";
    // Thread branch (t128b: T4/T5 = 32/64-thread cores removed — no such cores exist).
    ...
    // Parallel branch (t128b: 3 merged nodes).
    ...
    private CalculatorUpgradeTree() {
        ...
        // t128b (TEST ONLY): one iron per non-free node (the merged groups replace the old
        ...
        // Root (free) — t128b merged head: 机器基础 + 256k/1024k cells, auto-activated.
        addNode(new UpgradeNode(N1, k + N1 + ".name", k + N1 + ".effect", true));

        // Cell main chain N2..N5 (each depends on the previous). t128b: N2 = old {4096k,16M,64M},
        ...
        // N4, which is merged into new N2). t128b: T4/T5 (32/64-thread cores) removed.
        paid(iron, k, T1, N2);
        paid(iron, k, T2, T1);
        paid(iron, k, T3, T2);

        // Hyper-thread branch (H1 ← T2, H2 ← T3 unchanged; t128b: H3 prerequisite T3 → H2).
        paid(iron, k, H1, T2);
        paid(iron, k, H2, T3);
        paid(iron, k, H3, H2);

        // Parallel branch (t128b: P1 covers 1/4/16 (≤16), P2 64/256/1024 (≤1024), P3
        // 4096/16384/65536 (≤65536); head P1 branches off the new N2 like the thread branch).
        paid(iron, k, P1, N2);
        paid(iron, k, P2, P1);
        paid(iron, k, P3, P2);

        // Overclock terminal (反转彩蛋: 红线 5% + 超线程免额) — t128b prerequisites renamed:
    ...
    /** t128b helper: one paid (1 iron) node with the given prerequisites. */
