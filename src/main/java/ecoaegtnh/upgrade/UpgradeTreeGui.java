...
public final class UpgradeTreeGui {
     ...
     * <ul>
     * <li>calculator tree (has the OC terminal): cell main chain column x=57 (N1..N5, t131 merged
     ...
     * 48px apart, centered; t131: I1..I4 / F1..F5 / E1..E5 merged groups).</li>
     * </ul>
     */
    ...
    /** t70/t114/t131: calculator branch layout (columns N=57 / T=99 / H=141 / P=183 / B=225, rows 36px). */
    private static java.util.Map<String, int[]> calculatorPositions() {
        java.util.Map<String, int[]> pos = new java.util.LinkedHashMap<>();
        int x0 = 57;
        // t131: merged main chain N1 (free head) → N2 → N3 → N4 (16384M) → N5 (奇点晶阵).
        for (int i = 1; i <= 5; i++) {
        ...
        // Hyper-thread branch H1 ← T2 (row 2), H2 ← T3 (row 3), H3 ← H2 (row 4, t131).
        for (int i = 1; i <= 3; i++) {
            pos.put("H" + i, new int[] { x0 + 2 * COL_DX, 2 * ROW_DY + (i - 1) * ROW_DY });
        }
        // Parallel branch P1..P3 (t131 merged ≤16 / ≤1024 / ≤16777216) heads off the new N2.
        for (int i = 1; i <= 3; i++) {
    ...
    /**
     * t69/t70/t112/t114→t131: storage-tree positions — three independent vertical chains (I/F/E
     * columns x=72/120/168, 48px apart, rows 36px; group horizontally centered in the 280px
     * scroll area). t131: one node per MERGED capacity group — the item chain has 4 rows
    ...
    private static java.util.Map<String, int[]> storagePositions() {
        java.util.Map<String, int[]> pos = new java.util.LinkedHashMap<>();
        String[] chains = { "I", "F", "E" };
        int[] rows = { 4, 5, 5 }; // t131: merged groups per family chain.
        int x0 = 72;
