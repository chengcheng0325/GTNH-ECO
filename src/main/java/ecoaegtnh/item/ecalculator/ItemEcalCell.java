 ...
 * <p>
 * t131 (284, plan §3.3): the upgrade-tree gate maps the cell onto the merged main-chain node
 * N1..N5 (see {@link #getRequiredUpgradeNode()}).
 * <p>
...
public class ItemEcalCell extends Item {
    ...
    /**
     * t65/t114→t131 (upgrade tree, docs §2 revision): the cell main-chain node required to insert
     * this cell — one node per MERGED GROUP of three tiers (t131 284 merge, plan §3.3): 256k/1024k
     * → N1 (free base group), 4096k/16M/64M → N2, 256M/1024M/4096M → N3, 16384M → N4, Singularity
     * (奇点闪存晶阵) → N5 (Long.MAX_VALUE bytes, byte-pool cap lifted to unlimited). Activating the
