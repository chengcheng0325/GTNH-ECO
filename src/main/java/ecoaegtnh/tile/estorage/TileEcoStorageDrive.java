...
public class TileEcoStorageDrive extends TileEcoStoragePart implements IInventory, ISaveProvider {
    ...
    /**
     * t112→t131 (upgrade-tree gate, plan §3.4): node id required by the cell on its storage line —
     * ONE NODE PER MERGED CAPACITY GROUP of three cell sizes (256k..4096k → I1/F1/E1, 16M..256M →
    ...
    /**
     * t112→t131 (upgrade-tree restriction, plan §3.4): each cell needs its chain's MERGED GROUP
     * node — the k级 cells (256k..4096k) work from the free base node (I1/F1/E1★); 16M..256M →
     * I2/F2/E2, 1024M..16384M → I3/F3/E3, 人造宇宙 → I4/F4/E4, and the family-exclusive infinite
     * cells need F5/E5. When no controller is assembled the check is deferred (returns true) — the
     * multiblock formation check re-validates statically (t62) so the pre-assembly bypass is
     * closed.
     */
