...
/**
 * t35: E-Calculator parallel-core drive block (并行核心驱动器) — 1 slot holding an
 * {@code ItemEcalParallelCore}; the inserted core supplies parallelism to the host. Mirrors the
 * cell drive (t15 pattern): facing metadata 2-5 (vanilla furnace convention), per-face rendering
 * with a filled two-state front, shift+right-click insert/extract, breakBlock drops the core.
 */
public class BlockEcalParallelDrive extends Block implements ITileEntityProvider {
    ...
    public static BlockEcalParallelDrive INSTANCE;
    ...
    public BlockEcalParallelDrive() {
        super(Material.iron);
        setHardness(20.0F);
        setResistance(2000.0F);
        setStepSound(soundTypeMetal);
        setHarvestLevel("pickaxe", 2);
        setBlockName("ecoaegtnh.ecalculator_parallel_drive");
        setBlockTextureName("ecoaegtnh:ecal_parallel_drive"); // server-side / fallback texture
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        return new TileEcalParallelDrive();
    }
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (side == facingFromMeta(meta).ordinal()) {
            ...
            if (te instanceof ecoaegtnh.tile.ecalculator.TileEcalParallelDrive drive && drive.getCoreStack() != null) {
    ...
    public void registerBlockIcons(IIconRegister reg) {
        this.blockIcon = reg.registerIcon("ecoaegtnh:ecal_parallel_drive_front");
        iconFront = reg.registerIcon("ecoaegtnh:ecal_parallel_drive_front");
        iconFrontFilled = reg.registerIcon("ecoaegtnh:ecal_parallel_drive_front_filled");
        iconSide = reg.registerIcon("ecoaegtnh:ecal_parallel_drive");
    }
    ...
    /**
     * Shift-right-click interaction (t13 pattern, mirrors the cell drive): sneak + held parallel
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        TileEntity te = world.getTileEntity(x, y, z);
        if (!(te instanceof ecoaegtnh.tile.ecalculator.TileEcalParallelDrive drive)) {
            return false;
        }
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        // Drop the stored parallel core (with NBT) before the tile is destroyed (cell-drive
        // pattern, P1-3).
        TileEntity te = world.getTileEntity(x, y, z);
        if (te instanceof ecoaegtnh.tile.ecalculator.TileEcalParallelDrive drive) {
    ...
    public static BlockEcalParallelDrive register(String name) {
        INSTANCE = new BlockEcalParallelDrive();
