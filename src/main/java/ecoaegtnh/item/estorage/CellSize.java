...
public enum CellSize {
    ...
    /**
     * t131 (284, plan §3.4): the storage-tree node NUMBER for this size on its family chain — one
     * node per merged capacity GROUP (each node unlocks three cell tiers): k级 (256k/1024k/4096k)
     * → 1, M级 (16M/64M/256M) → 2, big-M (1024M/4096M/16384M) → 3, 人造宇宙 → 4, and the
     ...
     * <p>
     * Replaces t114d's per-size {@code chainIndex(StorageType)} — the merge made the per-family
     * chain position meaningless (a node now covers a whole group, not one size).
     */
