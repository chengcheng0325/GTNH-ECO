...
$files = @(
  ...
  'blocks\ecal_cell_drive_front.png','items\ecal_cell_c4.png'
)
# candidate origins: rows y in 3..7 / 163..167 / 323..327, cols x in 3..7 / 163..167 / 323..327 / 483..487
...
foreach ($f in $files) {
    ...
    "{0,-34} sim={1,4}%  slot=({2},{3})" -f $f, [math]::Round(100.0*$best.sim/256,1), $best.x0, $best.y0
}
