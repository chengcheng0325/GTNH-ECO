> [!WARNING]
> **安装、更新或卸载本模组前，请务必备份你的世界存档。**
> 本模组会向世界数据写入内容（多方块机器、存储盘内的物品、合成任务队列等）——一旦出现异常或需要回滚，没有备份将**无法恢复**。
> 客户端请备份 `saves` 目录；服务端请备份 `world` 目录（含 `level.dat` 与 `DIM*`）。
...
- **线程槽分配**：新任务按固定优先级占用线程槽——先内置普通线程槽，再外置普通线程槽，再内置超线程槽，最后外置超线程槽。
...
本项目基于 NovaEngineering-ECOAEExtension（sddsd2332、Kasumi_Nova、WI_8614_ice）的设计概念，
由 AI 辅助完成移植与开发：针对 GTNH 2.9.0-beta-2（AE2U rv3-beta-1000）与 GTNH 2.8.4
（AE2U rv3-beta-695）双版本适配，包括 vCPU 合成体系、存储阵列、故障安全与审计修复。
如有问题欢迎提交 Issue。
